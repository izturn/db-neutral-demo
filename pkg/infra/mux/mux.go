// modified from https://github.com/matryer/way
package mux
...
// muxContextKey is the context key type for storing
// parameters in context.Context.
type muxContextKey string

const allQueryValues = muxContextKey("all_query_values")
...
func (r *Router) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	for _, route := range r.routes {
		...
		if ctx, ok := route.match(req.Context(), r, segs); ok {
			...
			ctx = context.WithValue(ctx, allQueryValues, m)
...
func Param(ctx context.Context, param string) string {
	val, ok := ctx.Value(muxContextKey(param)).(string)
	if !ok {
		return ""
	}
	return val
...
// IntParam gets the path parameter from the specified Context and convert it to ingeger.
func IntParam(ctx context.Context, param string) (int, error) {
	return strconv.Atoi(Param(ctx, param))
}

// IntQuery gets the the first value for query from the specified Context and convert it to ingeger.
func IntQuery(ctx context.Context, query string) (int, error) {
	m := ctx.Value(allQueryValues).(map[string]string)
	return strconv.Atoi(m[query])
}
